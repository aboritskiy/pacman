...
public class TextString {
	...
	public TextString (Context context) {
		...
		stringDrawer = new StringDrawer(context);
	}
	
	public Boolean loadFont (String fileName, int height, int padX, int padY) {
		this.padX = padX;
		this.padY = padY;
		
		Log.d("TextString", "height: " + height);
...
//		GLES20.glBindTexture( GLES20.GL_TEXTURE_2D, 0 );// Unbind Texture
    ...
    // R: [none]
    public void draw(String text, float x, float y, float[] mProjMatrix, float[] mVMatrix)  {
        ...
        x += ( chrWidth / 2.0f ) - ( padX * scaleX );  // Adjust Start X
        y += ( chrHeight / 2.0f ) - ( padY * scaleY );  // Adjust Start Y
...
//	      batch.endBatch();                               // End Batch
	  ...
	  /* public void draw(float[] mProjMatrix, float[] mVMatrix) {
	        ...
	        stringDrawer.draw(mProjMatrix, mVMatrix);
	    }*/
